...
MyWindow::MyWindow(QWidget *parent) : QWidget(parent)
{
 ...
 liniaButton = new QPushButton("Linia");
 wypelnienieButton = new QPushButton("Wypelnienie");
 ...
 boxLayout->addWidget(liniaButton);
 ...
 connect(liniaButton,SIGNAL(clicked()),this,SLOT(slot_linia()));
 connect(wypelnienieButton,SIGNAL(clicked()),this,SLOT(slot_wypelnienie()));
...
void MyWindow::slot_wypelnienie(){
...
void MyWindow::floodFill(int x, int y, QImage *image)
{
    WspolrzednasQueue.enqueue({x, y});

    while (!WspolrzednasQueue.empty()) {

        Wspolrzedna obecnaWspolrzedna = WspolrzednasQueue.front();
        WspolrzednasQueue.dequeue();


        if (obecnaWspolrzedna.x < 0 || obecnaWspolrzedna.x >= szer || obecnaWspolrzedna.y < 0 || obecnaWspolrzedna.y >= wys)
            continue;


        unsigned char *ptr = img->bits();
        if (ptr[szer * 4 * obecnaWspolrzedna.y + 4 * obecnaWspolrzedna.x] == 255 &&
            ptr[szer * 4 * obecnaWspolrzedna.y + 4 * obecnaWspolrzedna.x + 1] == 255 &&
            ptr[szer * 4 * obecnaWspolrzedna.y + 4 * obecnaWspolrzedna.x + 2] == 255)
        {
            pixel(obecnaWspolrzedna.x, obecnaWspolrzedna.y, image);

            WspolrzednasQueue.enqueue({obecnaWspolrzedna.x - 1, obecnaWspolrzedna.y});
            WspolrzednasQueue.enqueue({obecnaWspolrzedna.x + 1, obecnaWspolrzedna.y});
            WspolrzednasQueue.enqueue({obecnaWspolrzedna.x, obecnaWspolrzedna.y + 1});
            WspolrzednasQueue.enqueue({obecnaWspolrzedna.x, obecnaWspolrzedna.y - 1});
        }
    }
}
